...
// Helper function to build connection profile
...
async function buildWallet() {
    ...
    // Check if user identity exists, if not use admin
    const userIdentity = await wallet.get(org1UserId);
    if (!userIdentity) {
        console.log(`🔄 User identity ${org1UserId} not found, using admin identity`);
        const adminIdentity = await wallet.get('admin');
        await wallet.put(org1UserId, adminIdentity);
        console.log(`✅ Created user identity ${org1UserId} from admin`);
    }
...
app.listen(PORT, async () => {
    console.log(`🚀 VeggieChain Blockchain Backend starting on port ${PORT}`);
    ...
    try {
        ...
        console.error('💡 Run: ./start-network.sh && ./deploy-chaincode.sh');
    }
});